...
{
    /* Vector table is the slot A image, not the 8 KiB header at 0x00440000. */
    boot_jump_xip(APP_START_ADDRESS);
}
 ...
 */
static const app_metadata_t *meta_from_slot_header(uint32_t hdr_addr)
{
    const uint8_t *base = (const uint8_t *)hdr_addr;
    const essfw_hdr_t *hdr = (const essfw_hdr_t *)hdr_addr;
    ...
    return (const app_metadata_t *)(base + offsetof(essfw_hdr_t, meta));
}

static const app_metadata_t *get_app_metadata(void)
{
    const app_metadata_t *meta;

    meta = meta_from_slot_header(SEC_BOOT_SLOT_A_HEADER_ADDR);
    if (meta != NULL) {
        return meta;
    }
    meta = meta_from_slot_header(SEC_BOOT_SLOT_B_HEADER_ADDR);
    if (meta != NULL) {
        return meta;
    }
    meta = (const app_metadata_t *)APP_METADATA_ADDR;
...
int32_t boot_print_board_info(void)
{
    ...
    snprintf(line_buf, sizeof(line_buf), "   Built Time:  %s\r\n",
             (meta->build_timestamp != NULL) ? meta->build_timestamp : "-");
    osal_log_info(line_buf);
    snprintf(line_buf, sizeof(line_buf), "   App Type:    Raw Binary\r\n");
    osal_log_info(line_buf);
    snprintf(line_buf, sizeof(line_buf), "   App Ver:     %s (F195)\r\n", version);
    osal_log_info(line_buf);
    ...
    osal_log_info("   Secure Boot: ECDSA-P256/SHA-256 (legacy fallback if no slot header)\r\n");
